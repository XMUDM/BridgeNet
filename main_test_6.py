...
#from model_egnn import ProductManifoldGNN
#from model_painn import ProductManifoldGNN
#from model_gvp import ProductManifoldGNN
...
def compute_atom_refs(dataset, target_idx, max_z=100):
    ...
    # 提取所有分子的原子序数和 Target 值
    # 注意：这里假设 dataset.data 全部在内存中，QM9 通常是这样的
    all_z = dataset.data.x[:, 0].long()
    all_batch = dataset.data.batch
    if all_batch is None:
        # 如果没有 batch 索引，我们需要手动生成
        # 这种情况比较少见，通常 PyG 的 InMemoryDataset 会有 batch 或者是 slice
        # 如果报错，请告知。这里假设处理后的 dataset.data 是由 DataLoader 拼接的大图逻辑
        # 或者我们需要遍历 dataset
        ...
        for i in tqdm(range(len(dataset))):
            ...
            targets.append(data.y[0, target_idx].item()) # 假设 y 是 [1, task_num]
        ...
        # 计算每个分子中各原子的数量
        # shape: [num_molecules, max_z]
        # 这需要一些技巧，简单起见还是用上面的迭代法最稳妥，
        # 或者使用 scatter_add 统计
        num_mols = y.size(0)
        A = torch.zeros(num_mols, max_z, device=y.device)
        ones = torch.ones_like(all_z, dtype=torch.float)
        # 将 ones 按照 (batch, z) 两个维度 scatter 会比较麻烦
        # 还是推荐上面的迭代法，QM9 13万数据跑一次也就几秒钟
        pass 

    # 最小二乘法求解 Ax = y => x = (A^T A)^-1 A^T y
    # x 即为每个原子类型的平均能量
    try:
    ...
    for step, batch in enumerate(loader):
        ...
        if batch.x.shape[0] == 1:
            ...
            if "classification" in task_type:
                ...
                # [核心修改点] QM9 使用 L1 Loss 计算验证集 Loss
                if dataset_name == 'qm9':
...
def main():
    ...
    parser.add_argument('--patience', default=200, type=int, help='max early stopping round')
    parser.add_argument('--dataset', type=str, default="Freesolv", help='BBBP, Bace, ClinTox, Tox21, Sider'
                                                                    'Freesolv, Lipop, Esol')
    # parser.add_argument('--num_path', default=82, type=int, help='16, 12, 45, 82, 12, 12, 18, 17')
    # parser.add_argument('--num_layers', default=1, type=int)
    parser.add_argument('--weight_decay', default=0.001, type=float, help='weight_decay on optimizer.')#0.0001 5e-5
    parser.add_argument('--output_dir', default='./', type=str)
    parser.add_argument('--eval_metric', type=str, default="rocauc")
    # parser.add_argument('--hidden_irreps', type=str, default="8x0e + 8x1o")
    ...
    # param = nni.get_next_parameters()
    # args.dropout = param['dropout']
    # args.hidden_dim = param['hidden_dim']
    ...
    # [shutil.rmtree(os.path.join('pool/Tox21', f)) if os.path.isdir(os.path.join('pool/Tox21', f))
    #  else os.remove(os.path.join('pool/Tox21', f)) for f in os.listdir('pool/Tox21')]
    ...
    # print(dataset.data.frag_h.shape, dataset.data.frag_edge_index)
    # exit()

    # print(dataset.data.y[:, args.target].unsqueeze(1).shape)
    # exit()
    ...
    # target = args.target
    # print(args.target)
    # dataset.data.y = dataset.data[args.target]
    # dataset.data.y = dataset.data.y[:, args.target].unsqueeze(1)
    ...
    if args.dataset in ["BBBP", "ClinTox", "Tox21", "Sider", "HIV",
        ...
        # [关键修复] 强制将索引转换为 long 类型，防止 IndexError
        train_idx = split_idx['train'].long()
        valid_idx = split_idx['valid'].long()
    ...
    if args.dataset in ["qm9", "Esol", "Freesolv", "Lipop"]:
        ...
        if args.dataset == "qm9":
            ...
            # [关键修改] 1. 找出所有出现过的原子类型 (列和不为0)
            col_sum = A.sum(dim=0) # [100]
            present_atom_indices = torch.nonzero(col_sum).squeeze() # e.g. [1, 6, 7, 8, 9]
            
            print(f"Atoms present in dataset (Z): {present_atom_indices.tolist()}")
            
            # [关键修改] 2. 只选取这些列构建子矩阵 A_sub
            A_sub = A[:, present_atom_indices] # [N_samples, N_present_atoms]
            
            # [关键修改] 3. 求解子问题 A_sub * x_sub = y
            try:
                # 使用 rcond=None 让 pytorch 自动处理一些病态情况
            ...
            # 打印一下计算出的基准能量，看看是否正常 (应该是负数，且绝对值较大)
    ...
    # 准备存储所有 Seed 的预测结果用于集成
    all_seed_test_preds = [] 
    test_y_true = None 

    n_seeds = args.seeds
    # 自定义 Seed 列表 (如果需要固定随机性)
    ...
    # [新增 2] ---------------- 将 Mean/Std 注入模型 ----------------
    if mean is not None and std is not None:
        # 将 mean 和 std 注册为模型的 buffer，这样它们会自动跟随模型移动到 GPU
    ...
    # [新增] 初始化 EMA 模型
    # 0.999 是常用的衰减率，意味着它保留了很长的历史信息，非常平滑
    ...
    # [重要] 确保 EMA 模型里也有 mean 和 std (如果使用了归一化)
    ...
    for i, seed in enumerate(custom_seeds):
        ...
        for epoch in pbar:
            ...
            # [修改 4] 保存模型时，传入 ema_model，这样保存的就是平滑后的权重
        ...
        # [修改] 关键修复：
        # 因为保存的是 ema_model (AveragedModel)，权重里有 "module." 前缀
        # 所以必须加载回 ema_model，而不是原始的 model
        ...
        # all_seed_test_preds.append(final_test_y_pred) # [移除] 不需要存储预测结果了
